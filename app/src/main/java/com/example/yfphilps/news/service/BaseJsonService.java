...
import com.example.yfphilps.news.utils.NetTask;
import com.example.yfphilps.news.utils.UrlUtil;
...
/*
	protected MemberVO getMemberVO() {
		if (mContext instanceof BaseActivity) {
			return ((BaseActivity) mContext).getApplicationSet().getMemberVO();
		}
		return null;
	}*/
		...
		NetTask netTask = new NetTask(param, "utf-8", Constants.METHOD_POST,
				new NetTask.INetComplete() {
					...
					public void complete(boolean isSuc, String content) {
						...
						completeresult(isSuc, jsoncontent);
					}
				});
